...
# Prompt to enter a password for the PostgreSQL filmapi user (rather than hard-coding
# a password in this script).
read -p "Enter password for filmapi DB user: " DB_PASSWORD
...
# Set up the filmapi DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE filmapi"
sudo -i -u postgres psql -d filmapi -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d filmapi -c "CREATE ROLE filmapi WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the filmapi database to the system-wide environment 
# variables in the /etc/environment file.
echo "FILMAPI_DB_DSN='postgres://filmapi:${DB_PASSWORD}@localhost/filmapi'" >> /etc/environment
